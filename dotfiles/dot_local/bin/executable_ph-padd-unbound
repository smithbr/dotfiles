...
GetUnboundStats() {
  # Use stats_noreset so we don't wipe cumulative counters
  local raw
  raw=$(sudo /usr/sbin/unbound-control stats_noreset 2>/dev/null)

  if [ -z "$raw" ]; then
    unbound_status="offline"
    unbound_queries="N/A"
    unbound_cache_hits="N/A"
    unbound_cache_misses="N/A"
    unbound_cache_hit_pct="N/A"
    unbound_avg_recursion="N/A"
    unbound_uptime="N/A"
    return
  fi
  ...
  local total queries cachehits cachemiss uptime_sec avg_recursion

  total=$(echo "$raw"      | awk -F= '/^total\.num\.queries=/{print $2}')
  cachehits=$(echo "$raw"  | awk -F= '/^total\.num\.cachehits=/{print $2}')
  cachemiss=$(echo "$raw"  | awk -F= '/^total\.num\.cachemiss=/{print $2}')
  avg_recursion=$(echo "$raw" | awk -F= '/^total\.recursion\.time\.avg=/{printf "%.0fms", $2 * 1000}')
  uptime_sec=$(echo "$raw" | awk -F= '/^time\.up=/{printf "%.0f", $2}')

  unbound_queries="${total:-0}"
  unbound_cache_hits="${cachehits:-0}"
  unbound_cache_misses="${cachemiss:-0}"
  unbound_avg_recursion="${avg_recursion:-N/A}"

  # Cache hit percentage
  if [ "${total:-0}" -gt 0 ] 2>/dev/null; then
    unbound_cache_hit_pct=$(awk "BEGIN{printf \"%.1f\", (${cachehits:-0}/${total})*100}")
  else
    unbound_cache_hit_pct="0.0"
  fi
...
PrintDashboard() {
    ...
    if [ "$1" = "pico" ]; then
        ...
        # ---- UNBOUND section (add inside your PrintRegularDisplay or PrintLargeDisplay function) ----
        moveXOffset; printf "%s${clear_line}\n" "${bold_text}UNBOUND ================================${reset_text}"
        moveXOffset; printf " %-9s${green_text}%-29s${reset_text}${clear_line}\n" "Status:" "${unbound_status}"
        moveXOffset; printf " %-9s%-29s${clear_line}\n" "Queries:" "${unbound_queries}"
        moveXOffset; printf " %-9s%-14s %-5s%-9s${clear_line}\n" "Cache:" "Hit: ${unbound_cache_hits}" "Miss:" "${unbound_cache_misses}"
        moveXOffset; printf " %-9s%-29s${clear_line}\n" "Hit%:" "${unbound_cache_hit_pct}%"
        moveXOffset; printf " %-9s%-29s${clear_line}\n" "Avg Rec:" "${unbound_avg_recursion}"
        moveXOffset; printf " %-9s%-29s${clear_line}\n" "Uptime:" "${unbound_uptime}"
        moveXOffset; printf "%s${clear_line}\n" "${bold_text}SYSTEM =====================================================${reset_text}"
        moveXOffset; printf " %-10s%-39s${clear_line}\n" "Uptime:" "${system_uptime}"
